...
void inputs() {
    if (Serial.available() > 0) {
        ...
        if (input == 'q') {
            ...
            digitalWrite(44, LOW);
            Serial.println("f");
        } // va avanti di 1 casella stampa f alla fine
        else if (input == 's') {
            digitalWrite(44, HIGH);
            movimento('s');
            digitalWrite(44, LOW);
            Serial.println("f");
        } // va indietro di 1 casella stampa f alla fine
        else if (input == 'a') {
            digitalWrite(44, HIGH);
            movimento('a');
            digitalWrite(44, LOW);
            Serial.println("f");
        } // gira di 90 gradi stampa f alla fine
        else if (input == 'd') {
            digitalWrite(44, HIGH);
            movimento('d');
            digitalWrite(44, LOW);
            ...
            digitalWrite(44, LOW);
            Serial.println("f");
        } // usato per prova (puoi metterci quello ch vuoi nella funzione test)
        else if (input == 'p') {
            digitalWrite(44, HIGH);
            pinga();
            digitalWrite(44, LOW);
            Serial.println("f");
        } // stampa la tettura dei sensori con f alla fine e all'inizio
        else if (input == 'f') {
            motori(0, 0, 0, 0, HIGH, HIGH, HIGH, HIGH, HIGH, HIGH, HIGH, HIGH);
            Serial.println("f");
        } // ferma il robot I.C.E. e stampa f
        else if (input == 'r') {
            digitalWrite(44, HIGH);
            resetFunc();
        } // chiama un reset quando arduino si impianta cosi non dobbiamo tornare al checkpoint e raspy lo resetta
        else {
            digitalWrite(44, HIGH);
            delay(100);
            digitalWrite(44, LOW);
        ...
        } //non ha capito cosa ha mandato e scrive errore
    }
}
...
void sensori() {
    ...
    for (int i = 0; i < 4; i++) {
        for (int y = 0; y < 2; y++) {
            ...
            digitalWrite(pingarray[i][j], LOW); //pin portato a low (per scamaranzia)
            delayMicroseconds(5); //dalay 5 microsecondi
            digitalWrite(pingarray[i][j], HIGH); //pin portato a high (inizio emisione impulso)
            delayMicroseconds(10); //delay di 10 secondi (nonchè durata approssimativa dell'impulso)
            digitalWrite(pingarray[i][j], LOW); //pin portato a low
            microspulse = pulseIn(pingarray[i][j], HIGH); //calcolo durata ricezzione impulso
            temp = dht.readTemperature(); //lettura temperatura
            vsuono = 331.3 + (0.606 * temp); //calcolo della velocità del suono in m/s
            media[i][j] = microspulse * (vsuono / 10000) / 2; //calcolo distanza in cm:
...
void startsequence() {
    ...
    distanza += distanza * erroredistanza; // do una percentuale di errore
    ...
    do {
        ...
        mpu6050.update();
        vel = velocita - (abs(mpu6050.getAngleZ() - angoloiniziale) * kAng);
    } while (ultrasuoni(pingArray[3][0]) - ultrasuoni(pingArray[3][1]) <= 0.5 || ultrasuoni(pingArray[3][0]) - ultrasuoni(pingArray[3][1]) >= -0.5);

    angolostandard = abs(mpu6050.getAngleZ() - angoloiniziale);

    Serial.println(angolostandard);
